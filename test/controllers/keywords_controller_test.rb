...
class KeywordsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create keyword" do
    assert_difference('Keyword.count') do
      post keywords_url, params: { keyword: { name: @keyword.name, order: @keyword.order, owner_id: @keyword.owner_id } }
    end
  ...
  test "should update keyword" do
    patch keyword_url(@keyword), params: { keyword: { name: @keyword.name, order: @keyword.order, owner_id: @keyword.owner_id } }
